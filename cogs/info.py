...
class Info(commands.Cog):
    ...
    async def help_command(self, ctx, cog='all'):
        ...
        if cog == 'all':
            ...
            help_embed.add_field(
                name="Moderation", value=f"`{ctx.prefix}help moderator`")
            help_embed.add_field(
                name="Images", value=f"`{ctx.prefix}help image`")
            help_embed.add_field(
                name="Utility", value=f"`{ctx.prefix}help utility`")
            help_embed.add_field(name="Info", value=f"`{ctx.prefix}help info`")
            help_embed.add_field(
                name="Music", value=f"`{ctx.prefix}help music`")
            help_embed.add_field(
                name="Other", value=f"`{ctx.prefix}help other`")
        else:
            cogA = cog.lower()
            emojiCategory = {"Moderator": ":tools:", "Image": ":camera:", "General": ":tools:",
                             "Info": ":question:", "Other": "", "Music": ":musical_note:", "Fun": ":wink:"}
            categoryAlias = {"mod": "moderator", "moderation": "moderator", "moderate": "moderator",
    ...
    async def botinvite(self, ctx):
        ...
        invite_embed.add_field(name=f"Full Permissions",
                               value=f"{https://discord.com/api/oauth2/authorize?client_id=566193825874182164&permissions=8&scope=bot}")
    ...
    @commands.command(
        ...
        aliases=['server', 'supportserver',
                 'helpserver', 'helpinvite', 'devserver']
    )
